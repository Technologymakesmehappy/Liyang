...
public class MySprite {
	...
	public void init(Sprite[] sprites, Material material)
	{
		...
		for (var i = 0; i < sprites.Length; ++i) {
			size_list_[i].x = sprites[i].textureRect.xMax - sprites[i].textureRect.xMin;
			size_list_[i].y = sprites[i].textureRect.yMax - sprites[i].textureRect.yMin;
		}
	...
	public void put(int front, float x, float y, float ratio, Kind kind, Type type)
	{
		int idx = (int)kind;
